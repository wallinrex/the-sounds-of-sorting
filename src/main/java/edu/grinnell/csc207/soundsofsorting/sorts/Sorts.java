...
public class Sorts {
    ...
    private static <T extends Comparable<? super T>> List<SortEvent<Integer>> mergeSort(
    ...
    private static <T extends Comparable<? super T>> List<SortEvent<Integer>> merge(
    ...
    private static <T extends Comparable<? super T>> List<SortEvent<Integer>> quickSort(
            T[] arr, int lo, int hi) {
        if (hi <= lo + 1) {
            return;
        }
        int mid = (hi - lo) / 2 + lo;
        int pivotIndex = medianIndex(arr, lo, mid, hi);
    ...
    private static <T extends Comparable<? super T>> int medianIndex(
        ...
        T[] temp = (T[]) new Object[] {arr[lo], arr[mid], arr[hi]};
        selectionSort(temp);
        if (temp[1] == arr[lo]) {
            return lo;
